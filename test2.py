...
def parse_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=2e-5)  # Higher LR for GELU
    parser.add_argument('--weight_decay', type=float, default=0.1)
    parser.add_argument('--warmup_ratio', type=float, default=0.1)
    parser.add_argument('--max_length', type=int, default=512)
    parser.add_argument('--dropout', type=float, default=0.3)  # Lower dropout for GELU
...
def highlight_target(sentence: str, homonym: str) -> str:
    ...
    # Last resort: return original sentence
    return sentence
...
def train_one_epoch(model, dataloader, optimizer, scheduler, device, grad_accum, use_amp):
    """Training loop with Huber loss + label smoothing for GELU"""
    model.train()
    total_loss = 0.0
